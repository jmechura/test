...
export class RoutingTableEffect {
  ...
  @Effect() getRoutingTables(): Observable<Action> {
    return this.actions$
  ...
  @Effect() postRoutingTable(): Observable<Action> {
    return this.actions$
      .ofType(routingTableActions.ROUTING_TABLE_API_POST)
      .switchMap(action => this.api.post(ROUTING_TABLE_ENDPOINT, action.payload)
        .map(res => ({type: routingTableActions.ROUTING_TABLE_POST, payload: res}))
        .catch(() => Observable.of({type: routingTableActions.ROUTING_TABLE_POST_FAIL}))
      );
  }

  @Effect() putRoutingTable(): Observable<Action> {
    return this.actions$
      .ofType(routingTableActions.ROUTING_TABLE_API_PUT)
      .switchMap(action => this.api.put(ROUTING_TABLE_ENDPOINT, action.payload)
        .map(res => ({type: routingTableActions.ROUTING_TABLE_PUT, payload: res}))
        .catch(() => Observable.of({type: routingTableActions.ROUTING_TABLE_PUT_FAIL}))
      );
  }
  ...
  @Effect() deleteRoutingTable(): Observable<Action> {
    return this.actions$
      ...
      .switchMap(action => this.api.remove(`${ROUTING_TABLE_ENDPOINT}/${action.payload}`)
        .map(res => ({type: routingTableActions.ROUTING_TABLE_DELETE, payload: action.payload}))
        .catch(() => Observable.of({type: routingTableActions.ROUTING_TABLE_DELETE_FAIL}))
      );
  }
}
